...
import "./Blogs.css";
function Blogs({ onBack, onCreatedBlog, editPost, isEditing }) {
  ...
  const handleImageChange = (e) => {
    if (e.target.files && e.target.files[0]) {
      ...
      const maxSlice = 1 * 1024 * 1024;

      if (file.size > maxSlice) {
        alert("File sze excedds 1 MB");
        return;
      }
  ...
  const handleSubmit = (e) => {
    ...
    onCreatedBlog(newBlog, isEditing);
    setImage(null);
    setTitle("");
    setContent("");
    setShowForm(false);
  ...
  return (
    <div className="blogs">
      ...
      <div className="blogs-right">
        ...
        {submitted && <p className="submitted-message">Post Submitted</p>}
        <div className={`blogs-right-form ${showForm ? "visible" : "hidden"}`}>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <button className="submit-btn" type="submit">
              {isEditing ? "Updated Post" : "Submit Post"}
            </button>
          </form>
        </div>
